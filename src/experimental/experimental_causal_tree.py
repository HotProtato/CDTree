...
class UnsupervisedCausalTree:
    """
    An experimental tree, that aims to maximize (best score - 2nd best score) / (parent node score).

    Results indicate greater stability across different hyperparameters, pending further testing.
    """
    ...
    def _find_best_split(self, X):
        ...
        parent_total_impurity = sum(parent_impurities.values()) if parent_impurities else 1.0 # Avoid division by zero
        ...
        for feature_index in subspace_indices:
            ...
            for threshold in thresholds:
                ...
                original_score = (1 - sum_pos_gain - sum_abs_neg_gain) * (sum_pos_gain + sum_abs_neg_gain)**self.beta

                if original_score >= self.min_score_threshold and original_score <= self.max_score_threshold:
                    candidate_splits.append({
                        'feature_index': feature_index, 'threshold': threshold, 'original_score': original_score,
                        'impact_matrix': np.zeros((n_features, n_features)), # Initialize, will fill later
                        'gains': gains,
        ...
        # Sort candidates by their original score to find best and second best
        candidate_splits.sort(key=lambda x: x['original_score'], reverse=True)

        best_original_score = candidate_splits[0]['original_score']
        second_best_original_score = candidate_splits[1]['original_score'] if len(candidate_splits) > 1 else 0.0

        # Apply the new scoring criterion
        if parent_total_impurity == 0: # Handle cases where all features have 0 impurity
            new_score = best_original_score # Fallback to original score if no impurity to normalize by
        else:
            new_score = (best_original_score - second_best_original_score) / parent_total_impurity
        
        # Select the best split based on the new score (which is derived from the original best)
        # We still return the split details of the original best candidate
        best_split_candidate = candidate_splits[0]
        
        impact_matrix = np.zeros((n_features, n_features))
        impact_matrix[best_split_candidate['feature_index'], :] = best_split_candidate['gains']

        return {
            'feature_index': best_split_candidate['feature_index'],
            'threshold': best_split_candidate['threshold'],
            'score': new_score, # Use the new score for the split
            'impact_matrix': impact_matrix,
            'left_indices': best_split_candidate['left_indices'],
            'right_indices': best_split_candidate['right_indices']
        }
